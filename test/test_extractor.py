...
def test_athena_extractor(open_catalog_connection):
    ...
    with catalog.managed_session:
        ...
        assert (
            ...
            == """awsathena+rest://access_key:secret_key@athena.us_east_1.amazonaws.com:443/
            ?s3_staging_dir=staging_dir&aws_session_token=aws_session_token
            &mfa_serial=mfa"""
        )
...
def test_athena_extractor_iam(open_catalog_connection):
    ...
    with catalog.managed_session:
        ...
        assert (
            scoped.get_string(SQLAlchemyExtractor.CONN_STRING)
            == """awsathena+rest://:@athena.us_east_1.amazonaws.com:443/
            ?s3_staging_dir=staging_dir
            &aws_sessiontoken=aws_session_token&mfa_serial=mfa"""
        )
